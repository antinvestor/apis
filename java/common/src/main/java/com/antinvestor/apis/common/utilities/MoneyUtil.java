...
public class MoneyUtil {
    ...
    public static Money from(BigDecimal amount, String currency) {
        ...
        int nanos =
                amount.subtract(new BigDecimal(units))
                        .multiply(NumberUtils.NANO_DIVISOR)
                        .intValue();
    ...
    public static BigDecimal toBigDecimal(Money money) {
        return new BigDecimal(money.getUnits())
                .add(
                        new BigDecimal(money.getNanos())
                                .divide(NumberUtils.NANO_DIVISOR, NumberUtils.mathContext()));
    }
    ...
    public static Money multiply(Money a, BigDecimal b) {
        ...
        if (Objects.isNull(b)) {
            throw new UnRetriableException(
                    STATUSCODES.BAD_REQUEST_DATA_ERROR, "Attempting to multiply with null number ");
        }
    ...
    public static Money divide(Money a, BigDecimal b) {
        ...
        if (Objects.isNull(b)) {
            throw new UnRetriableException(
                    STATUSCODES.BAD_REQUEST_DATA_ERROR, "Attempting to divide with null number ");
        }
    ...
    public static String format(Money money) {
        return String.format(
                ...
                toBigDecimal(money).setScale(2, RoundingMode.DOWN).toPlainString());
    }
    ...
    public static void validateCurrenciesMatch(Money a, Money b) {
        if (!a.getCurrencyCode().equalsIgnoreCase(b.getCurrencyCode())) {
            throw new UnRetriableException(
                    STATUSCODES.BAD_CURRENCY_ERROR, "Amounts have different currencies");
        }
    }
    ...
    public static void validateMoney(Money... moneys) {
        for (Money money : moneys) {
            if (Objects.isNull(money)) {
                throw new UnRetriableException(
                        STATUSCODES.BAD_DATE_ERROR, "Money object must not be null");
            }

            if (TextUtils.isBlank(money.getCurrencyCode())) {
                throw new UnRetriableException(
                        STATUSCODES.BAD_CURRENCY_ERROR,
                        "Money object must have a valid currency set");
            }
        }
    }
}
