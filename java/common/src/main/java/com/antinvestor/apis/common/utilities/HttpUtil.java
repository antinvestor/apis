...
public class HttpUtil {
    ...
    public static HttpUtil instance() {
        ...
        var gson = new GsonBuilder().setFieldNamingPolicy(LOWER_CASE_WITH_UNDERSCORES).create();
    ...
    protected HttpRequest.Builder enrich(String uri, HttpRequest.Builder requestBuilder)
            throws UnRetriableException, RetriableException {
        return requestBuilder;
    }

    public <T> T post(Type responseType, String uri, Object body)
            throws RetriableException, UnRetriableException {
        return post(responseType, uri, body, true);
    }

    public <T> T post(Type responseType, String uri, Object body, boolean enrich)
            throws RetriableException, UnRetriableException {
    ...
    public <T> T patch(Type responseType, String uri, Object body)
            throws RetriableException, UnRetriableException {
    ...
    public <T> T put(Type responseType, String uri, Object body)
            throws RetriableException, UnRetriableException {
    ...
    public <T> T get(Type responseType, String uri)
            throws RetriableException, UnRetriableException {

        HttpRequest.Builder requestBuilder = newRequestBuilder().GET().uri(URI.create(uri));
    ...
    public <T> T delete(Type responseType, String uri)
            throws RetriableException, UnRetriableException {

        HttpRequest.Builder requestBuilder = newRequestBuilder().DELETE().uri(URI.create(uri));
    ...
    protected void handleError300s(HttpRequest httpRequest, HttpResponse<String> response)
            throws RetriableException {
        // This is a redirect request so we will just follow the redirect if available.
        log.warn(
                " invoke -- Request was redirected to {} from {}",
                response.body(),
                httpRequest.uri());
        throw new RetriableException(STATUSCODES.BAD_RESPONSE_DATA_ERROR, response.body());
    }

    protected void handleError400s(HttpResponse<String> response)
            throws UnRetriableException, RetriableException {
    ...
    protected void handleErrorOthers(HttpResponse<String> response) throws RetriableException {
        ...
        throw new RetriableException(
                STATUSCODES.UNSPECIFIED_ERROR_WHILE_PROCESSING, response.body());
    }

    private <T> T invoke(Type responseType, HttpRequest httpRequest)
            throws RetriableException, UnRetriableException {
        return invoke(responseType, httpRequest, true);
    }

    private <T> T invoke(Type responseType, HttpRequest httpRequest, boolean canRetry)
            throws RetriableException, UnRetriableException {

        log.debug(
                " invoke -- Invoking the url {} using {} with headers {}",
                httpRequest.uri(),
                httpRequest.method(),
                httpRequest.headers());
