...
import build.buf.gen.files.v1.FilesServiceClient;
...
@ApplicationScoped
public class FilesClient extends ConnectClientBase<FilesServiceClient> {
    ...
    protected ConnectionConfig getConnectionConfig(Context context, DefaultConfig defaultConfig) {
        ...
        return new ConnectionConfig(
                cfg.filesHostUrl(), cfg.filesHostPort(), cfg.authInterceptorEnabled());
    }

    public FilesServiceClient stub(Context context) {
        return new FilesServiceClient(getProtocolClient());
    }
}
