...
@ApplicationScoped
public class LedgerClient extends ConnectClientBase<LedgerServiceClient> {
    ...
    protected ConnectionConfig getConnectionConfig(Context context, DefaultConfig defaultConfig) {
        ...
        return new ConnectionConfig(
                cfg.ledgerHostUrl(), cfg.ledgerHostPort(), cfg.authInterceptorEnabled());
    }

    public LedgerServiceClient stub(Context context) {
        return new LedgerServiceClient(getProtocolClient());
    }
    ...
    public Iterable<Ledger> listLedgers(Context context, String jsonQuery) {
        SearchRequest searchRequest = SearchRequest.newBuilder().setQuery(jsonQuery).build();
        var result =
                this.<SearchRequest, SearchLedgersResponse>executeServerStream(
                        context,
                        stub(context),
                        searchRequest,
                        (client, headers, continuation) ->
                                client.searchLedgers(headers, continuation));
        return IteratorUtil.flatMapIterable(result, SearchLedgersResponse::getDataList);
    }

    public Ledger createLedger(
            Context context, String parent, LedgerType type, Map<String, Object> data) {

        CreateLedgerRequest.Builder ledgerBuilder = CreateLedgerRequest.newBuilder().setType(type);
        ...
        var result =
                this.<CreateLedgerRequest, CreateLedgerResponse>executeUnary(
                        context,
                        stub(context),
                        ledgerBuilder.build(),
                        (client, request, headers, continuation) ->
                                client.createLedger(request, headers, continuation));
        return result.getData();
    }

    public Optional<Ledger> updateLedger(
            Context context, String reference, Map<String, Object> data) {
        Optional<Ledger> optionalLedger = getLedger(context, reference);
        if (optionalLedger.isPresent()) {
            var updateLedgerRequest =
                    UpdateLedgerRequest.newBuilder()
                            .setId(reference)
                            .setData(ProtoStructUtil.fromMap(data))
                            .build();
            var result =
                    this.<UpdateLedgerRequest, UpdateLedgerResponse>executeUnary(
                            context,
                            stub(context),
                            updateLedgerRequest,
                            (client, request, headers, continuation) ->
                                    client.updateLedger(request, headers, continuation));
    ...
    public Iterable<Account> listAccounts(Context context, String jsonQuery) {
        SearchRequest searchRequest = SearchRequest.newBuilder().setQuery(jsonQuery).build();
        var result =
                this.<SearchRequest, SearchAccountsResponse>executeServerStream(
                        context,
                        stub(context),
                        searchRequest,
                        (client, headers, continuation) ->
                                client.searchAccounts(headers, continuation));
    ...
    public Account createAccount(
        ...
        CreateAccountRequest.Builder accountBuilder =
        ...
        var result =
                this.<CreateAccountRequest, CreateAccountResponse>executeUnary(
                        context,
                        stub(context),
                        accountBuilder.build(),
                        (client, request, headers, continuation) ->
                                client.createAccount(request, headers, continuation));
        return result.getData();
    }
    ...
    public Optional<Account> updateAccount(
        ...
        UpdateAccountRequest.Builder accountBuilder =
                UpdateAccountRequest.newBuilder()
                        .setId(reference)
                        .setData(ProtoStructUtil.fromMap(data));

        var response =
                this.<UpdateAccountRequest, UpdateAccountResponse>executeUnary(
                        context,
                        stub(context),
                        accountBuilder.build(),
                        (client, request, headers, continuation) ->
                                client.updateAccount(request, headers, continuation));
    ...
    public Iterable<Transaction> listTransactions(Context context, String jsonQuery) {
        SearchRequest searchRequest = SearchRequest.newBuilder().setQuery(jsonQuery).build();
        var result =
                this.<SearchRequest, SearchTransactionsResponse>executeServerStream(
                        context,
                        stub(context),
                        searchRequest,
                        (client, headers, continuation) ->
                                client.searchTransactions(headers, continuation));
        return IteratorUtil.flatMapIterable(result, SearchTransactionsResponse::getDataList);
    }

    public Transaction createTransaction(Context context, TransactionBuilder transactionBuilder)
            throws UnRetriableException {

        CreateTransactionRequest transaction = transactionBuilder.build();

        var result =
                this.<CreateTransactionRequest, CreateTransactionResponse>executeUnary(
                        context,
                        stub(context),
                        transaction,
                        (client, request, headers, continuation) ->
                                client.createTransaction(request, headers, continuation));
        return result.getData();
    }
    ...
    public Optional<Transaction> clearTransaction(Context context, String reference) {
        ...
        transactionBuilder.setClearedAt(
                LocalDateTime.now(ZoneOffset.UTC).format(DateTimeFormatter.ISO_DATE_TIME));
        var nueTransaction = transactionBuilder.build();
        var result =
                this.<UpdateTransactionRequest, UpdateTransactionResponse>executeUnary(
                        context,
                        stub(context),
                        nueTransaction,
                        (client, request, headers, continuation) ->
                                client.updateTransaction(request, headers, continuation));
        return Optional.of(result.getData());
    }
    ...
    public Optional<Transaction> updateTransaction(
        ...
        if (optionalTransaction.isPresent()) {
            ...
            UpdateTransactionRequest.Builder transactionBuilder =
                    UpdateTransactionRequest.newBuilder();
    ...
    public Optional<Transaction> reverseTransaction(Context context, String reference) {
        ...
        ReverseTransactionRequest transaction =
                ReverseTransactionRequest.newBuilder().setId(reference).build();

        var response =
                this.<ReverseTransactionRequest, ReverseTransactionResponse>executeUnary(
                        context,
                        stub(context),
                        transaction,
                        (client, request, headers, continuation) ->
                                client.reverseTransaction(request, headers, continuation));
        return Optional.of(response.getData());
    }
    ...
    public Iterable<TransactionEntry> listTransactionEntries(Context context, String jsonQuery) {
        SearchRequest searchRequest = SearchRequest.newBuilder().setQuery(jsonQuery).build();
        var results =
                this.<SearchRequest, SearchTransactionEntriesResponse>executeServerStream(
                        context,
                        stub(context),
                        searchRequest,
                        (client, headers, continuation) ->
                                client.searchTransactionEntries(headers, continuation));
