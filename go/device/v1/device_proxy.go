...
func CreateProxyHandler(ctx context.Context, proxyOptions common.ProxyOptions) (*http.ServeMux, error) {
	...
	err := RegisterDeviceServiceHandlerFromEndpoint(ctx, implementationMux, proxyOptions.GrpcServerEndpoint, proxyOptions.GrpcServerDialOpts)
	if err != nil {
		return nil, err
	}

	err = proxyOptions.ServeAPISpec(proxyMux, apiSpecFs, "device.swagger.json")
	if err != nil {
		return nil, err
	}
