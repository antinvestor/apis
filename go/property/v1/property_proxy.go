...
//go:embed property.swagger.json
...
func CreateProxyHandler(ctx context.Context, proxyOptions common.ProxyOptions) (*http.ServeMux, error) {
	...
	err := RegisterPropertyServiceHandlerFromEndpoint(ctx, implementationMux, proxyOptions.GrpcServerEndpoint, proxyOptions.GrpcServerDialOpts)
	if err != nil {
		return nil, err
	}

	err = proxyOptions.ServeAPISpec(proxyMux, apiSpecFs, "property.swagger.json")
	if err != nil {
		return nil, err
	}
