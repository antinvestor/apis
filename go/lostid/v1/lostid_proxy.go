...
func CreateProxyHandler(ctx context.Context, proxyOptions common.ProxyOptions) (*http.ServeMux, error) {
	...
	err := RegisterLostIdServiceHandlerFromEndpoint(ctx, implementationMux, proxyOptions.GrpcServerEndpoint, proxyOptions.GrpcServerDialOpts)
	if err != nil {
		return nil, err
	}

	err = proxyOptions.ServeAPISpec(proxyMux, apiSpecFs, "lostid.swagger.json")
	if err != nil {
		return nil, err
	}
