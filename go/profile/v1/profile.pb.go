...
const file_profile_v1_profile_proto_rawDesc = "" +
	...
	"profile.v1\x1a\x1bbuf/validate/validate.proto\x1a\x16common/v1/common.proto\x1a\x1cgoogle/protobuf/struct.proto\x1a$gnostic/openapi/v3/annotations.proto\"\xc5\x02\n" +
	"\rContactObject\x12+\n" +
	...
	"\fBLACK_LISTED\x10\x022\xc6#\n" +
	"\x0eProfileService\x12\xdc\x01\n" +
	"\aGetById\x12\x1a.profile.v1.GetByIdRequest\x1a\x1b.profile.v1.GetByIdResponse\"\x97\x01\xbaG\x93\x01\n" +
	"\bProfiles\x12\x11Get profile by ID\x1adRetrieves a complete profile by its unique identifier including contacts, addresses, and properties.*\x0egetProfileById\x12\xe0\x01\n" +
	"\fGetByContact\x12\x1f.profile.v1.GetByContactRequest\x1a .profile.v1.GetByContactResponse\"\x8c\x01\xbaG\x88\x01\n" +
	"\bProfiles\x12\x16Get profile by contact\x1aORetrieves a profile associated with a specific contact (email or phone number).*\x13getProfileByContact\x12\x8a\x02\n" +
	"\x06Search\x12\x19.profile.v1.SearchRequest\x1a\x1a.profile.v1.SearchResponse\"\xc6\x01\xbaG\xc2\x01\n" +
	"\bProfiles\x12\x0fSearch profiles\x1a\x94\x01Searches for profiles matching specified criteria including name, contact, date range, and custom properties. Returns a stream of matching profiles.*\x0esearchProfiles0\x01\x12\xe1\x01\n" +
	"\x05Merge\x12\x18.profile.v1.MergeRequest\x1a\x19.profile.v1.MergeResponse\"\xa2\x01\xbaG\x9e\x01\n" +
	"\bProfiles\x12\x0eMerge profiles\x1asMerges two profiles by combining their data. The merge source profile data is incorporated into the target profile.*\rmergeProfiles\x12\xda\x01\n" +
	"\x06Create\x12\x19.profile.v1.CreateRequest\x1a\x1a.profile.v1.CreateResponse\"\x98\x01\xbaG\x94\x01\n" +
	"\bProfiles\x12\x0eCreate profile\x1aiCreates a new profile with the specified type (person, institution, bot) and initial contact information.*\rcreateProfile\x12\xda\x01\n" +
	"\x06Update\x12\x19.profile.v1.UpdateRequest\x1a\x1a.profile.v1.UpdateResponse\"\x98\x01\xbaG\x94\x01\n" +
	"\bProfiles\x12\x0eUpdate profile\x1aiUpdates an existing profile's properties and state. Contacts and addresses are managed via separate RPCs.*\rupdateProfile\x12\x8a\x02\n" +
	"\n" +
	"AddContact\x12\x1d.profile.v1.AddContactRequest\x1a\x1e.profile.v1.AddContactResponse\"\xbc\x01\xbaG\xb8\x01\n" +
	...
	"addContact\x12\xfb\x01\n" +
	"\rCreateContact\x12 .profile.v1.CreateContactRequest\x1a!.profile.v1.CreateContactResponse\"\xa4\x01\xbaG\xa0\x01\n" +
	"\bContacts\x12\x19Create standalone contact\x1ajCreates a standalone contact that can later be linked to a profile. Useful for pre-registration scenarios.*\rcreateContact\x12\xc1\x02\n" +
	"\x19CreateContactVerification\x12,.profile.v1.CreateContactVerificationRequest\x1a-.profile.v1.CreateContactVerificationResponse\"\xc6\x01\xbaG\xc2\x01\n" +
	"\bContacts\x12\x1bCreate contact verification\x1a~Initiates contact verification by sending a verification code via email or SMS. The code expires after the specified duration.*\x19createContactVerification\x12\x96\x02\n" +
	"\x11CheckVerification\x12$.profile.v1.CheckVerificationRequest\x1a%.profile.v1.CheckVerificationResponse\"\xb3\x01\xbaG\xaf\x01\n" +
	"\bContacts\x12\x17Check verification code\x1awVerifies a contact by checking the provided verification code. Tracks verification attempts and returns success status.*\x11checkVerification\x12\xe2\x01\n" +
	"\rRemoveContact\x12 .profile.v1.RemoveContactRequest\x1a!.profile.v1.RemoveContactResponse\"\x8b\x01\xbaG\x87\x01\n" +
	"\bContacts\x12\x0eRemove contact\x1a\\Removes a contact from a profile. The contact is disassociated but may remain in the system.*\rremoveContact\x12\x94\x02\n" +
	"\fSearchRoster\x12\x1f.profile.v1.SearchRosterRequest\x1a .profile.v1.SearchRosterResponse\"\xbe\x01\xbaG\xba\x01\n" +
	"\x06Roster\x12\rSearch roster\x1a\x92\x01Searches a user's contact roster (contact list) with filtering by date range, properties, and custom criteria. Returns a stream of roster entries.*\fsearchRoster0\x01\x12\xd9\x01\n" +
	"\tAddRoster\x12\x1c.profile.v1.AddRosterRequest\x1a\x1d.profile.v1.AddRosterResponse\"\x8e\x01\xbaG\x8a\x01\n" +
	"\x06Roster\x12\x12Add roster entries\x1aaAdds multiple contacts to a user's roster (contact list). Each contact is verified automatically.*\taddRoster\x12\xf8\x01\n" +
	"\fRemoveRoster\x12\x1f.profile.v1.RemoveRosterRequest\x1a .profile.v1.RemoveRosterResponse\"\xa4\x01\xbaG\xa0\x01\n" +
	"\x06Roster\x12\x13Remove roster entry\x1asRemoves a contact from a user's roster (contact list). The profile remains but is no longer in the user's contacts.*\fremoveRoster\x12\xcc\x01\n" +
	"\n" +
	"AddAddress\x12\x1d.profile.v1.AddAddressRequest\x1a\x1e.profile.v1.AddAddressResponse\"\x7f\xbaG|\n" +
	"\tAddresses\x12\vAdd address\x1aVAdds a new physical address to a profile with optional geocoding (latitude/longitude).*\n" +
	"addAddress\x12\x88\x02\n" +
	"\x0fAddRelationship\x12\".profile.v1.AddRelationshipRequest\x1a#.profile.v1.AddRelationshipResponse\"\xab\x01\xbaG\xa7\x01\n" +
	"\rRelationships\x12\x10Add relationship\x1asCreates a relationship between two profiles (member, affiliated, blacklisted). Supports hierarchical relationships.*\x0faddRelationship\x12\x84\x02\n" +
	"\x12DeleteRelationship\x12%.profile.v1.DeleteRelationshipRequest\x1a&.profile.v1.DeleteRelationshipResponse\"\x9e\x01\xbaG\x9a\x01\n" +
	"\rRelationships\x12\x13Delete relationship\x1a`Removes an existing relationship between profiles. The profiles remain but are no longer linked.*\x12deleteRelationship\x12\xaa\x02\n" +
	"\x10ListRelationship\x12#.profile.v1.ListRelationshipRequest\x1a$.profile.v1.ListRelationshipResponse\"\xc8\x01\xbaG\xc4\x01\n" +
	"\rRelationships\x12\x12List relationships\x1a\x8b\x01Lists all relationships for a profile with optional filtering by type and related profiles. Supports pagination and relationship inversion.*\x11listRelationships0\x01B\xb6\x06\xbaG\x85\x05\x12\xd9\x04\n" +
	...
	"\x1fcom.antinvestor.apis.profile.v1B\fProfileProtoP\x01Z3github.com/antinvestor/apis/go/profile/v1;profilev1\xa2\x02\x03PXX\xaa\x02\n" +
	"Profile.V1\xca\x02\n" +
